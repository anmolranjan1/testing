...
export default function EmployeeTypeTab() {
  ...
  const loadData = useCallback(async (pageNum = 1, searchTerm = "") => {
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      ...
      if (lastSearchedTermRef.current === trimmedSearch) {
        return;
      }

      lastSearchedTermRef.current = trimmedSearch;
      loadData(1, trimmedSearch);
    }, 350);
